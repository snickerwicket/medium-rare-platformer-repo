function Level(plan) {
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      // Get the type from that character in the string. It can be '@', 'x', '!', 'y', or ' '
      // If the character is ' ', assign null.

      var ch = line[x], fieldType = null;
		
	  if (ch === '@')
		//create new player at this grid position
...
function Player (pos){
	...
	//to make an autorunner, the player would start with speed
...
DOMDisplay.prototype.drawPlayer = function(){
	...
	//be sure to leave the space after actor, as we are sending two things.
